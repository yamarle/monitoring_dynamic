...
int main(int argc, char** argv) {
    ...
    int nextWaypointInd = 0; // Índice (de la posisción del camino) del siguiente waypoint a alcanzar
    ...
    //while(ros::ok()){ // Hasta que llegue al objetivo
        if(newGoal){
            // El primer punto del camino es el más cercano al robot
            //goalGridPosition.first = path2Goal.x[0]; goalGridPosition.second = path2Goal.y[0];
            ...
            cout<<"LONGITUD DEL CAMINO: "<<length<<" - con pathStep: "<<pathStep<<endl;
            //goalGridPosition.first = path2Goal.x[nextWaypointInd]; goalGridPosition.second = path2Goal.y[nextWaypointInd];
            goalGridPosition.first = path2Goal.y[nextWaypointInd]; goalGridPosition.second = path2Goal.x[nextWaypointInd];
            // Transformo a las coordenadas del mapa
            //goalPose = multi_util::transformFromGridCoordinates(map, goalGridPosition);
            goalPose = path2GoalPoses[nextWaypointInd]; // Selección del pose del goal
            newGoal = false;
            if(!goalRx){
                //sendGoal(goalPose.position.x, goalPose.position.y);
                //sendGoal(goalPose.position.y, goalPose.position.x); // No tengo en cuenta la orientación
        ...
        cout<<"Expl → Goal: ("<<currPose.position.x<<", "<<currPose.position.y<<", "<<tf::getYaw(currPose.orientation)<<") → ("<<goalPose.position.x<<", "<<goalPose.position.y<<", "<<tf::getYaw(goalPose.orientation)<<")"<<endl;
        //cout<<"Expl → Pose: ("<<currPose.position.x<<", "<<currPose.position.y<<"): "<<tf::getYaw(currPose.orientation)<<endl;
        if(dist <= MIN_PROXIMITY_RANGE){ // HE LLEGADO AL OBJETIVO
            //path2Goal.pop(); // Quito el waypoint de la lista
            ...
            if(pathStep > path2GoalPoses.size()){
                path2Goal.subPath_(pathStep, path2Goal.x.size());
                path2GoalPoses.erase(path2GoalPoses.begin(), path2GoalPoses.begin() + pathStep);
            }else{
        ...
        laserGridPos = multi_util::transform2GridCoordinates(map, laserGridPoses);
        laserPoss = points2Poss(laserGridPos);
        //cout<<"Las posiciones del laser"<<endl; laserPoss.show_();
        ...
        laserGridCell.header.stamp = ros::Time::now();
        //cout<<laserGridCell.cells.size()<<" cells"<<endl;
    ...
    //sendGoal(goalPosition.first, goalPosition.second);
