...
void computeVelocitiesDWA(const geometry_msgs::Pose currentPose, const geometry_msgs::Pose goalPose, const sensor_msgs::LaserScan laserData, double max_angular_speed, double max_linear_speed, double robot_radius, double& linearVel, double& angularVel)
{
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber occGridSub = nh.subscribe<nav_msgs::OccupancyGrid>("/map", 1, boost::bind(multi_util::gridCallback, _1, boost::ref(mapMsg), boost::ref(occGrid), boost::ref(grid)));
    ...
    /*
    ros::Subscriber laser_Sub = nh.subscribe<sensor_msgs::LaserScan>("/base_scan", 1, &laserCallback);
    ros::Subscriber pose_Sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose", 1, &robotPoseCallback);
    */
    ...
    */
    ...
    if(grid.size()){
        ...
        navArea = multi_util::navigableGradient(obstGrad, 0.5/mapMsg.info.resolution);
    ...
    int vrange = (int)(30.0/mapMsg.info.resolution); // Rango del laser (RECUERDA MIRAR EN STAGE) // ←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←
    ...
    while(ros::ok() && explore){ // Hasta que explore todo el espacoio libre
        ...
        sh_vect_h(laser_poss_values,"laser_values");
        visible_obstacles = newObstacles(laserPoss, laser_poss_values); // Posiciones de los nuevos obstáculos
        if(visible_obstacles.x.size()){
            cout<<"--------------------------> OBSTÁCULOS NUEVOS"<<endl;
            visible_obstacles.show();
            for(int i=0; i<visible_obstacles.x.size(); i++){
                //navArea[visible_obstacles.x[i]][visible_obstacles.y[i]] = 0;
                agent_ = agentPoss(visible_obstacles.x[i], visible_obstacles.y[i], ax, ay);
                for(int j=0; j<agent_.x.size(); j++){
                    navArea[agent_.x[j]][agent_.y[j]] = 0;
                }
            }
            //cin.get();
        }
        ...
        if(path2Goal.x.size()){
            ...
            //Poss<int> agent_in_goal_obstacles = newObstacles(agent_, agent_in_goal_values);
