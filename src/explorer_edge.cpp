...
int main(int argc, char** argv) {
    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        */
        ...
        expl_obj.recompute_target(gridPosition.first, gridPosition.second, areas_graph, distance_between_segments, segment_obs_points, tree, tree_costs, branch);
        ...
        //if(path2Goal.tam){
            ...
            for(int i=0; i<branch.size()-1; i++){
                Poss<int> p = bresenham::points(fmm_segments.centroids.x[branch[i]], fmm_segments.centroids.y[branch[i]], fmm_segments.centroids.x[branch[i+1]], fmm_segments.centroids.y[branch[i+1]]);
                for(int j=0; j<p.x.size()-1; j++){
                    path2Goal.push_back(p.x[j], p.y[j]);
                }
            }
        //}
        ...
        // Averiguar si hay que enviar el camino que se ha calculado
        if(path2Goal.tam){ // Se ha obtenido camino
            /*
            sendPath = false;
            // Obtener los segmentos que se van a atravesar
            pathSegs = possInSegments(path2Goal.x, path2Goal.y, fmm_segments.map, fmm_segments.centroids.x.size());
            sh_vect_h(pathSegs,"segmentos atravesados");
            // Comprobar si hay obstáculos dinámicos dentro de estos segmentos
            //pathInDynAreas = dynamicAreasInSegs(pathSegs, fmm_segments.map, areas.poss);
            pathInDynAreas = dynamicObstInSegs(pathSegs, fmm_segments.map, visible_obstacles);
            sh_vect_h(pathInDynAreas,"Con obstáculos");
            // Obtener la distancia "estimada" hasta el segmento con obstáculos
            d2os = dist2obstSeg(pathSegs, pathInDynAreas, maxSegDist);

            if(d2os < 0){ // No atraviesa ningún segmento con dinamismo
                cout<<"camino libre de obstáculos"<<endl;
                sendPath = false; // el camino ya se ha enviado -> no enviar
            }else if(d2os == 0){ // Estoy en un segmento en el que se están moviendo obstáculos dinámicos
                cout<<"comparto segmento con obstáculo"<<endl;
                sendPath = true;
            }else{ // El camino atraviesa segmentos en el que se mueven obstáuclos dinámicos
                cout<<"hay obstáculos por el camino"<<endl;
                sendPath = true;
            }

            cout<<"Distancia estimada al obstáculo: "<<d2os<<endl;
            */

            /*
            if(visible_obstacles.x.size()){ // Veo obstáculos nuevos
                //Comprobar a qué distancia se encuentran y enviar el camino
                for(int i=0; i<visible_obstacles.x.size(); i++){
                    // 5: parar y [recalcular + girar + moverme] mientras el obstáculo se mueve hacia el robot
                    if(hypot(gridPosition.first - visible_obstacles.x[i], gridPosition.second - visible_obstacles.y[i]) <= 5*robotRadius){
                        // Hay obstáculos cerca del robot, se envía el camino nuevo
                        sendPath = true; // Se envia el camino nuevo
                        break;
                    }
                }
                // No hay obstáculos cerca
                // Compruebo si el camino atraviesa las mismas zonas que antes
                if(!sendPath){
                    sendPath = changesInTraversedSegments(pathSegs, _pathSegs);
                }
            }else{ // No hay obstáculos nuevos
                sendPath = changesInTraversedSegments(pathSegs, _pathSegs);
            }
            */
    ...
    vector<float> res_vect = {multi_util::computePathDistance(totalPath), precomputationTime, missionTime, explored};
