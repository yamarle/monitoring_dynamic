...
void _updateNavigableMap(vector<vector<float>> &navigable_map, vector<vector<float>> original_map, Poss<int> laser_poss, Poss<int> all_laser_poss, int ax, int ay, int x, int y, float vrange)
{
    ...
    for(int i=0; i<laser_poss.x.size(); i++){
        if(original_map[laser_poss.x[i]][laser_poss.y[i]]){ // Estoy viendo un obstáculo que no está en el mapa estático
            ...
            //cout<<"RANGO: "<<d<<" < "<<vrange<<endl;
            //cout<<abs(d-vrange)<<" < "<<2*mapMsg.info.resolution<<endl;
...
void computeVelocitiesAcc(const geometry_msgs::Pose goalPose, double dObstMaxNorm, double& linearVel, double& angularVel)
{
    ...
    double angleDiff = atan2(goalPose.position.y - currPose.position.y, goalPose.position.x - currPose.position.x) - tf::getYaw(currPose.orientation);
...
int main(int argc, char** argv) {
    ...
    vector<int> branch, branch_orig, _branch;
    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        _updateNavigableMap(navArea, navAreaOrig, laserPoss, visible_poss, ax, ay, gridPosition.first, gridPosition.second, vrange);
        ...
        // almacenar el instante en el que se han visto por última vez las posiciones
        assign_value(visible_poss, currentTime, last_seen_map);
        visible_map = visibleMap(visible_obstacles, gsx, gsy);

        // Comprobar si los obstáculos que veo son reales o es un error
        //for(int i=0; i<clusters.size(); i++){
        int ic = 0;
        while(ic<clusters.size()){
            int jc = 0;
            Poss<int> cluster_poss = clusters[ic];
            while(jc<cluster_poss.x.size()){
                if(last_seen_map[cluster_poss.x[jc]][cluster_poss.y[jc]] == currentTime){
                    // Lo que estoy viendo actualmente
                    if(visible_map[cluster_poss.x[jc]][cluster_poss.y[jc]]){
                        jc++;
                    }else{
                        cluster_poss.erase(jc);    
                    }
                }else if(last_seen_map[cluster_poss.x[jc]][cluster_poss.y[jc]] < currentTime){
                    // Posiciones que he dejado de ver
                    cluster_poss.erase(jc);
                }
            }
            if(!cluster_poss.x.size()){
                for(int i=0; i<clusters[ic].x.size(); i++)
                    obstacles_trajectories_points.erase_point(clusters[ic].x[i], clusters[ic].y[i]);
                clusters.erase(clusters.begin() + ic);
            }else{
                ic++;
            }
        }

        _visible_map = visible_map;
        
        //save_matr("visible_obstacles_map.txt", seen_map);
        save_matr("visible_obstacles_map.txt", last_seen_map);
        */
        // -------------------------------------------------------------------
        ...
        //cout<<fmm_segments.map[gridPosition.first][gridPosition.second]<<" -> "<<expl_obj.get_target_area()<<endl;
        ...
        // Calculo el árbol y la rama una vez
        expl_obj.compute_target(gridPosition.first, gridPosition.second, areas_graph, distance_between_segments, segment_obs_points, tree, tree_costs, branch);
        branch_orig = branch;
        sh_vect_h(branch_orig, "RAMA COMPLETA");
        //cout<<"ÁREA OBJETIVO: "<<expl_obj.get_target_area()<<endl;
        cout<<"Robot: "<<fmm_segments.map[gridPosition.first][gridPosition.second]<<" -> "<<expl_obj.get_target_area()<<endl;

        // Borro el área en la que ya se encuentra el robot
        if(branch.size() && branch[0]==fmm_segments.map[gridPosition.first][gridPosition.second]){
            branch.erase(branch.begin());
        }

        if(branch.size()>1){
            path2Goal.clear();
            // Compruebo que no haya oclusiones entre el robot y el centroide del área siguiente
            bool LoS = true;
            Poss<int> p = bresenham::points(gridPosition.first, gridPosition.second, fmm_segments.centroids.x[branch[0]], fmm_segments.centroids.y[branch[0]]);
            for(int i=0; i<p.x.size(); i++){
                if(!navArea[p.x[i]][p.y[i]]){
                    LoS = false;
                    break;
                }
            }

            if(LoS){ // Hay línea de visión
                // Voy a ese punto
                path2Goal.push_back(fmm_segments.centroids.x[branch[0]], fmm_segments.centroids.y[branch[0]]);
            }else{ // No hay línea de visión
                // Calculo el camino a ese punto
                path2Goal = navigation::path2goal(gridPosition.first, gridPosition.second, navArea, fmm_segments.centroids.x[branch[0]], fmm_segments.centroids.y[branch[0]]);

                // Obtener los segmentos que atraviesa el camino
                _branch = select_values_from_matr(fmm_segments.map, path2Goal.x, path2Goal.y);
                _branch = unique_no_sort(_branch);
                sh_vect_h(_branch, "NUEVO CAMINO PASA POR");
                // Comparar las áreas atravesadas por las ramas
                bool recompute = false;
                if(_branch.size() > 2){
                    //cout<<"RAMA MÁS LARGA <------------------ "<<endl;
                    recompute = true;
                }else{
                    if(_branch.size() && _branch[1] != branch_orig[1]){
                        //cout<<"OTRA RAMA <------------------ "<<endl;
                        //recompute = true;
                    }
                }
                if(recompute && fmm_segments.map[gridPosition.first][gridPosition.second]>=0){
                    cout<<"CALCULO CAMINO PARA SEGUIR RAMA EVITANDO OBSTÁCULOS"<<endl;
                    vector<vector<float>> empty_grid(gsx, vector<float>(gsy,0));
                    //set_values_in_grid(empty_grid, fmm_segments.poss[fmm_segments.map[gridPosition.first][gridPosition.second]], (float)1);
                    //set_values_in_grid(empty_grid, fmm_segments.poss[branch_orig[1]], (float)1);
                    set_values_in_grid(empty_grid, fmm_segments.poss[fmm_segments.map[gridPosition.first][gridPosition.second]], select_values_from_matr(navArea, fmm_segments.poss[fmm_segments.map[gridPosition.first][gridPosition.second]].x, fmm_segments.poss[fmm_segments.map[gridPosition.first][gridPosition.second]].y));
                    set_values_in_grid(empty_grid, fmm_segments.poss[branch_orig[1]], select_values_from_matr(navArea, fmm_segments.poss[branch_orig[1]].x, fmm_segments.poss[branch_orig[1]].y));
                    path2Goal = navigation::path2goal(gridPosition.first, gridPosition.second, empty_grid, fmm_segments.centroids.x[branch[0]], fmm_segments.centroids.y[branch[0]]);
                    cout<<"camino: "<<path2Goal.tam<<endl;
                }
            }

            // Inserto el resto de los centroides de la rama en el camino
            //if(branch.size()>1)
            if(path2Goal.tam)
            for(int i=1; i<branch.size(); i++){
                path2Goal.push_back(fmm_segments.centroids.x[branch[i]], fmm_segments.centroids.y[branch[i]]);
            }
        }else if(branch.size() == 1){ // Me queda un único punto
            path2Goal.clear();
            path2Goal.push_back(gridPosition.first, gridPosition.second);
            path2Goal.push_back(fmm_segments.centroids.x[branch[0]], fmm_segments.centroids.y[branch[0]]);
        }

        if(path2Goal.tam>1 && path2Goal.tam < 4 && fmm_segments.map[gridPosition.first][gridPosition.second]>=0){
            // 
            if(hypot(gridPosition.first-path2Goal.x[0], gridPosition.second-path2Goal.y[0])>hypot(gridPosition.first-path2Goal.x[path2Goal.x.size()-1], gridPosition.second-path2Goal.y[path2Goal.y.size()-1])){
                path2Goal.pop_back();
                Poss<int> p = bresenham::points(gridPosition.first, gridPosition.second, fmm_segments.centroids.x[branch[0]], fmm_segments.centroids.y[branch[0]]);
                for(int i=1; i<p.x.size(); i++){
                    path2Goal.push_back(p.x[i], p.y[i]);
                }
            }
        }
        
        cout<<"CAMINO"<<endl; path2Goal.show();
        ...
        //cout<<"SIGO EXPLORANDO: "<<explore<<endl;
        ...
        //if(path2Goal.x.size() <= 2*robotRadiusGrid || hypot(gridPosition.first - path2Goal.x[path2Goal.x.size()-1], gridPosition.second - path2Goal.y[path2Goal.y.size()-1]) <= 4*robotRadiusGrid){ // Estoy en el objetivo
        //if(path2Goal.x.size() <= 2*robotRadiusGrid){ // Estoy en el objetivo
        if(hypot(currPose.position.x - path2Follow[path2Follow.size()-1].position.x, currPose.position.y - path2Follow[path2Follow.size()-1].position.y) <= 2*robotRadius){
        ...
        */
        ...
        norm_tree_costs = norm_v(tree_costs);

        //sh_vect_h(tree_costs,"Tcostes");
        //sh_vect_h(norm_tree_costs,"Ncostes");
        //sh_matr_al_(tree);

        // tree: aristas
        // tree_costs: costes asociados a los vértices del grafo (árbol)
        // norm_tree_costs: costes asociados a las aristas

        /*
        if(tree_costs.size()){
            int ind = 10;
            vector<vector<float>> _costs = {tree_costs, norm_tree_costs};
            _costs[0].erase(_costs[0].begin()+ind, _costs[0].end());
            _costs[1].erase(_costs[1].begin()+ind, _costs[1].end());
            sh_matr_al_(_costs);
        }
        cout<<tree.size()<<" - "<<tree_costs.size()<<" - "<<norm_tree_costs.size()<<endl;
        */
        ...
        /*
        // CALCULAR Y ENVIAR LOS COMANDOS DE VELOCIDAD DIRECTAMENTE DESDE AQUI
        double _goalDist = 0.0; // Distancia al goal máxima seleccionada en cada iteración
        double _d = 0.0;
        double _d2g = 0.0;

        cout<<"HAGO CALCULOS"<<endl;

        vector<geometry_msgs::Pose> _poses = path2Poses(followPath);

        if(_poses.size() > 1){
            geometry_msgs::Pose goalPose = selGoal_distances(currPose, _poses, _goalDist, _d, _d2g);
            if(_poses.size()){
                if(_d2g > 2*robotRadius) computeVelocitiesAcc(goalPose, 1, cmd_vel.linear.x, cmd_vel.angular.z);
                else computeVelocitiesAcc(goalPose, _d, cmd_vel.linear.x, cmd_vel.angular.z);
