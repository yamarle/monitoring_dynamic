...
int main(int argc, char** argv) {
    ...
    */
    ...
    while(ros::ok() && explore){ // Hasta que explore todo el espacoio libre
        ...
        if(laserPoss.x.size()){
            for(int i=0; i<laserPoss.x.size(); i++){
                navArea[laserPoss.x[i]][laserPoss.y[i]] = 0;
            }
        }
        //cout<<laser_poss_values.size()<<" - "<<map_values.size()<<endl;
        
        visible_obstacles = newObstacles(laserPoss, laser_poss_values); // Posiciones de los nuevos obstáculos
        // Incluyo los obstáculos en el mapa
        ...
        //visible_obstacles = newValuesinMap(laserPoss, laser_poss_values, navAreaOrig, visible_values); // Posiciones de lo "nuevo" que veo
        visible_obstacles = newValuesinMap(visible_poss, map_values, navAreaOrig, visible_values); // Posiciones de lo "nuevo" que veo
        //cout<<visible_obstacles.x.size()<<" - "<<visible_values.size()<<endl;
        // Incluyo los obstáculos en el mapa o quito los que dejo de ver
        if(visible_obstacles.x.size()){
            for(int i=0; i<visible_obstacles.x.size(); i++){
                //navArea[visible_obstacles.x[i]][visible_obstacles.y[i]] = 0;
                // Incluyo las posiciones del agente (volumen)
                agent_ = agentPoss(visible_obstacles.x[i], visible_obstacles.y[i], ax, ay);
                for(int j=0; j<agent_.x.size(); j++){
                    navArea[agent_.x[j]][agent_.y[j]] = visible_values[i];
                }
            }
            //cin.get();
        }
