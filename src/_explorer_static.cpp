...
vector<int> possInSegments(vector<Poss<int>> poss, vector<vector<int>> segmentsMap, int nsegments)
{
    ...
    return bool2ind(segments);   
} // Determinar los segmentos en los que se encuentran los puntos

vector<Poss<int>> _clusterPoss(Poss<int> poss, vector<vector<int>> segmentsMap, int nsegments, vector<int> &segs)
{
    vector<Poss<int>> res(nsegments);
    vector<bool> segments(nsegments, false);
    for(int i=0; i<poss.x.size(); i++){
        if(segmentsMap[poss.x[i]][poss.y[i]] >= 0 && segmentsMap[poss.x[i]][poss.y[i]] < nsegments-1){
            res[segmentsMap[poss.x[i]][poss.y[i]]].push(poss.x[i], poss.y[i]);
            segments[segmentsMap[poss.x[i]][poss.y[i]]] = true;
        }
    }
    segs = bool2ind(segments);
    return res;   
...
int main(int argc, char** argv) {
    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        */
        ...
        // Formar las áreas dinámicas a partir de los segmentos
        //areas = dynamic_areas::obtain_areas(clusters, static_map);
        //vector<int> dyn_segs = possInSegments(obstacles_trajectories_points, fmm_segments.map, fmm_segments.centroids.x.size());
        ...
        save_matr("dynAreas1.txt", areas.map);

        cout<<"áreas dinámicas obtenidas"<<endl;

        // -----------------------------------------------------------------------------------------------------------
        // Olvidar las áreas dinámicas

        if(forget_dynamic_areas){
            cout<<"OLVIDO ÁREAS"<<endl;
            ...
            for(int i=0; i<areas.N; i++){
                ...
                vector<int> areaInSegments = dyn_segs;
            ...
            cout<<"variables de las áreas eliminadas"<<endl;
            ...
            cout<<"áreas recalculadas"<<endl;

        }

        save_matr("dynAreas2.txt", areas.map);
        //save_matr("seen_time.txt", seen_time);

        cout<<"ESTOY EN "<<fmm_segments.map[gridPosition.first][gridPosition.second]<<" - "<<areas.map[gridPosition.first][gridPosition.second]<<endl;
        ...
        cout<<"Areas: "<<areas.N<<" -> "<<areas.poss.size()<<" - "<<areas.contours.size()<<" - "<<areas.contours_edges.size()<<" - "<<areas.map[gridPosition.first][gridPosition.second]<<endl;
        ...
        cout<<"EL ROBOT SE ENCUENTRA A "<<navArea[gridPosition.first][gridPosition.second]<<" DEL OBSTÁCULO"<<endl;
        ...
        //segCentrPoints.points = multi_util::buildGraphPoints(fmm_segments.centroids.y, fmm_segments.centroids.x, areas_graph, mapMsg); // Representar el grafo
        segCentrPoints.points = multi_util::buildTreePoints(fmm_segments.centroids.y, fmm_segments.centroids.x, tree, mapMsg); // Representar árbol
        ...
        cout<<"comparo obstáculos: "<<_visible_obstacles.x.size()<<" - "<<visible_obstacles.x.size()<<endl;
    ...
    vector<float> res_vect = {multi_util::computePathDistance(totalPath), precomputationTime, missionTime};
